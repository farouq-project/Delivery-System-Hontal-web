              ...
              <table className="w-full text-sm">
                <thead className="bg-gray-50">
                  <tr className="border-b border-gray-200">
                    ...
                    <th className="px-4 py-3 text-left text-xs font-bold text-gray-700">Cluster / Branch</th>
                    ...
                    <th className="px-4 py-3 text-right text-xs font-bold text-gray-700">Deliveries</th>
                    <th className="px-4 py-3 text-right text-xs font-bold text-gray-700">Success Rate</th>
                    <th className="px-4 py-3 text-right text-xs font-bold text-gray-700">Avg Order</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-100">
                  {clusters.map((row, i) => (
                    <tr key={String(row.cluster)} className="hover:bg-gray-50">
                      <td className="px-4 py-3 text-xs text-gray-400">{i + 1}</td>
                      <td className="px-4 py-3 font-semibold text-gray-900">{String(row.cluster)}</td>
                      <td className="px-4 py-3 text-right text-gray-800">{fmtIdr(Number(row.revenue))}</td>
                      <td className="px-4 py-3 text-right text-gray-800">{fmtNum(Number(row.total_orders))}</td>
                      <td className="px-4 py-3 text-right text-gray-800">{fmtNum(Number(row.deliveries))}</td>
                      ...
                      <td className="px-4 py-3 text-right text-gray-800">{fmtIdr(Number(row.avg_order))}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
